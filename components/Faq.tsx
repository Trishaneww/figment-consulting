...
const Faq = () => {
  return (
    <div className="flex flex-col justify-center mt-12 lg:mt-20 scroll-smooth py-24 pb-40 px-6 gap-6">
      ...
      <div className="flex flex-col justify-center items-center gap-6 text-slate-950 text-base md:text-lg lg:text-xl px-2 py-6 rounded-lg">
        <Accordion
          type="single"
          collapsible
          className="flex flex-col gap-2 w-full max-w-[850px]"
        >
          {faqData.map((accordion: any, idx: number) => (
            <AccordionItem
              value={`item-${idx}`}
              className="py-2 px-4 rounded-2xl bg-white"
              key={idx}
            >
              <AccordionTrigger className="text-base font-medium md:text-lg text-left">
                {accordion.item}
              </AccordionTrigger>
      ...
      <div className="flex flex-col gap-2 items-center px-6 lg:px-12 py-8 border-gray-500 border-[1px] rounded-lg text-center">
